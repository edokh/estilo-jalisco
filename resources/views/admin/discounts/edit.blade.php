            ...
            <form method="POST" action="{{ route('admin.discounts.update', $discount) }}"
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="w-full rounded-lg border border-[#D4A017]/20 bg-[#130E07] px-4 py-3 text-[#FAF3E0] placeholder:text-gray-500 focus:border-[#D4A017] focus:outline-none focus:ring-2 focus:ring-[#D4A017]/30">{{ old('description', $discount->description) }}</textarea>
                    ...
                    <select name="food_item_id"
                        ...
                        @foreach ($foodItems as $item)
                            <option value="{{ $item->id }}"
                                {{ old('food_item_id', $discount->food_item_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </select>

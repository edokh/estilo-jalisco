...
<div class="max-w-2xl mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-8">Edit Discount</h1>
    ...
    <form method="POST" action="{{ route('admin.discounts.update', $discount) }}"
        ...
        <div>
            ...
            <input type="text" name="name" required value="{{ old('name', $discount->name) }}"
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full px-4 py-2 border rounded">{{ old('description', $discount->description) }}</textarea>
        ...
        <div>
            ...
            <select name="food_item_id" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-orange-600">
                ...
                @foreach ($foodItems as $item)
                    <option value="{{ $item->id }}" {{ old('food_item_id', $discount->food_item_id) == $item->id ? 'selected' : '' }}>
                        {{ $item->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="value" step="0.01" required value="{{ old('value', $discount->value) }}"
                       class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-orange-600">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="datetime-local" name="start_date" required value="{{ old('start_date', $discount->start_date->format('Y-m-d\TH:i')) }}"
                       class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-orange-600">
            ...
            <div>
                ...
                <input type="datetime-local" name="end_date" required value="{{ old('end_date', $discount->end_date->format('Y-m-d\TH:i')) }}"
                       class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-orange-600">
        ...
        <div>
            <label class="flex items-center">
                <input type="checkbox" name="active" value="1" {{ old('active', $discount->active) ? 'checked' : '' }} class="mr-2">
